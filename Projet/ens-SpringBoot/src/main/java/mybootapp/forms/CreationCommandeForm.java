...
import mybootapp.beans.Commande;
import mybootapp.dao.ClientDao;
import mybootapp.dao.CommandeDao;
...
public final class CreationCommandeForm {
   ...
   public Commande creerCommande(HttpServletRequest request, String chemin) {
      ...
      if(ANCIEN_CLIENT.equals(choixNouveauClient)) {
          ...
          * 
          * L'objet métier pour valider la création d'un client existe déjà, il est donc déconseillé
          * de dupliquer ici son contenu ! À la place, il suffit de passer la requête courante à
          * l'objet métier existant et de récupérer l'objet Client créé.
          */
         CreationClientForm clientForm = new CreationClientForm(clientDao);
         client = clientForm.creerClient(request, chemin);

         /*
          * Et très important, il ne faut pas oublier de récupérer le contenu de la map d'erreur
          * créée par l'objet métier CreationClientForm dans la map d'erreurs courante, actuellement
          * vide.
          */
      ...
      /*
       * Ensuite, il suffit de procéder normalement avec le reste des champs spécifiques à une
       * commande.
       */
